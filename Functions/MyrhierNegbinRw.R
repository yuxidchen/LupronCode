########################################################
# Negative binomial HB model (author : )
# Author : Peter Rossi and Greg Allenby
# Update Notes (to make it work with real-world data): 
# Julia Liu 2013-12-03 : hess_i[[i]] = list(hess = diag(rep(10, nvar)) 
#                         when mle2$convergence != 0
# Julia Liu 2014-02-28 : Beta =  t(matrix(rep(Deltabar, nreg), ncol = nreg))   
#                        starts from Deltabar, instead of pooled result
# Julia Liu 2015-04-21 : added na.rm=T
########################################################
...
#    Beta = t(matrix(rep(beta_mle, nreg), ncol = nreg))
    Beta =  t(matrix(rep(Deltabar, nreg), ncol = nreg))   # starts from Deltabar, instead of pooled result
#print(Beta)
...
#            if (unif <= acc & betac[2] >=0 & betac[3] >=0 ) {
            if (unif <= acc ) {
                Beta[i, ] = betac
